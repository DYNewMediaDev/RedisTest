...
public class JedisTest {
	...
	public void test() {
		...
		try {
			...
			System.out.println("key:" + jedisCacheService.get("key"));
		} catch (Exception e) {
	...
	public void sortedSet() {
		...
		System.out.println(jedis.zcount("zset", 5, 5));
		// 整个集合值
		System.out.println(jedis.zrange("zset", 0, -1));
	}
	 ...
	 * 我们自增一个key，很容易得到一个独一无二的自增ID，然后通过此ID创建对象–为对象的每个字段设置一个key。
	 * 最后将新对象的ID压入submitted.news list。
	 * 这只是牛刀小试。在命令参考文档中可以读到所有和list有关的命令。你可以删除元素，旋转list，根据索引获取和设置元素，
	 * 当然也可以用LLEN得到list的长度。
	 */
	 ...
	 * 要获取一个对象的所有标签，如此简单： $ redis-cli smembers news:1000:tags 5 1 77 2
	 * 而有些看上去并不简单的操作仍然能使用相应的Redis命令轻松实现。例如我们也许想获得一份同时拥有标签1, 2,
